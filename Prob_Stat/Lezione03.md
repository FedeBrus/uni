...
## Regole
### Regola 1
Se $A$ è un evento con $Pr(A)$ allora $Pr(A^{c})=1-Pr(A)$.
...
\emptyset = A \cap A^{c}
$$
Per la (3) abbiamo che $Pr(\Omega) = Pr(A \cup A^{c}) = Pr(A) + Pr(A^{c})$. Dalla (2) sappiamo che $Pr(\Omega) = 1$, da cui segue eche $Pr(A^{c}) = 1- Pr(A)$.

Nota:
$\emptyset = \Omega^{c}$, quindi $Pr(\emptyset) = 1 -Pr(\Omega) = 1-1 = 0$
...
$$
P(A) = P \left( \bigcup_{\omega_{i} \in \mathcal{A}}\{ \omega_{i} \} \right) = \sum_{\omega_{i}\in \mathcal{A}} P(\{ \omega_{i} \}) = \sum_{\omega_{1} \in \mathcal{A}} p(\{ \omega_{i} \})
$$
Nota:
