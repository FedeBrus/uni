...
### Dimostrazione
Procediamo per induzione su $n\geq_{0}$: $P(n):=(\text{"enunciato con n variabile"})$. Quindi $P(n)$ è un'affermazione singola che in realtà nasconde infinite affermazioni, per l'arbitrarietà di $m$.
...
Sia $g(n):I_{n} \rightarrow X$ una bigezione e sia $x_{n} := g(n)$.
...
Caso 2:
$f(Y) \ni x_{n}$. Poiché $f$ è iniettiva, $\exists!y_{n}\in Y : f(y_{n})=x_{n}$.
...
### Teorema 
$(\mathbb{N}, \leq)$ sono ben ordinati, ovvero ogni sottoinsieme $A$ non vuoto di $\mathbb{N}$ ammette un minimo.